  ...
  isKoreanBusinessNumber: (businessNumber: string): boolean => {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      if (digit === undefined || weight === undefined) {
        return false; // Should not happen with length check
      }
      sum += parseInt(digit, 10) * weight;
    }
    
    const lastDigit = cleaned[8];
    if(lastDigit === undefined) return false;

    const checkDigitCalc = (sum + (parseInt(lastDigit, 10) * 5) / 10) % 10;
    
    const finalDigit = cleaned[9];
    if(finalDigit === undefined) return false;

    return checkDigitCalc === parseInt(finalDigit!, 10);
  },
